...
"""
import argparse, os, subprocess, json, urllib.request
...
def heuristic_summary(diff_text: str) -> str:
    ...
    title = f"Draft: Update {files[0]} (+{added}/-{removed}, {len(files)} files)" if files else "Draft: Update codebase"

    bullets = []
    if added or removed:
        bullets.append(f"Code changes: **+{added} / -{removed}** across **{len(files)} file(s)**.")
    if subsystems:
...
"""
    data = {
        "model": model,
        "messages": [
            {"role": "system", "content": "You are an expert release engineer. Be precise and concise."},
