...
def test_entry_points(script_runner):
    ...
    assert ret.success
    assert subprocess.run([PY_CMD, '-m', 'pretext', '-h'], shell=True).returncode == 0

def test_version(script_runner):
    ret = script_runner.run(PTX_CMD, '--version')
...
def test_build(tmp_path:Path,script_runner):
    assert script_runner.run(PTX_CMD, 'new', '-d', '.', cwd=tmp_path).success
    assert script_runner.run(PTX_CMD, 'build', 'web', cwd=tmp_path).success
    assert (tmp_path/'output'/'web').exists()
    assert script_runner.run(PTX_CMD, 'build', 'subset', cwd=tmp_path).success
    ...
    assert script_runner.run(PTX_CMD, 'build', '-g', cwd=tmp_path).success
    assert (tmp_path/'generated-assets').exists()
    os.removedirs(tmp_path/'generated-assets')
    assert script_runner.run(PTX_CMD, 'build', '-g', 'webwork', cwd=tmp_path).success
    assert (tmp_path/'generated-assets').exists()

def test_init(tmp_path:Path,script_runner):
    assert script_runner.run(PTX_CMD, 'init', cwd=tmp_path).success
    ...
    assert script_runner.run(PTX_CMD, 'init', '-r', cwd=tmp_path).success
...
def test_generate(tmp_path:Path,script_runner):
    ...
    assert script_runner.run(PTX_CMD,'generate','asymptote', cwd=tmp_path).success
    assert (tmp_path/'generated-assets'/'asymptote'/'test.html').exists()
    os.remove(tmp_path/'generated-assets'/'asymptote'/'test.html')
    assert script_runner.run(PTX_CMD,'generate','-x', 'test', cwd=tmp_path).success
    assert (tmp_path/'generated-assets'/'asymptote'/'test.html').exists()
    os.remove(tmp_path/'generated-assets'/'asymptote'/'test.html')
