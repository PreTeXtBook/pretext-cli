...
class Cursor:
    ...
    def at_page_start(self):
        return (self.chars_left == self.text_width) and (
            self.lines_left == self.page_height
        )
...
class BRF:
    ...
    def adjust_text_width(self, adjustment):
        ...
        if adjustment < 0:
            assert (
                self.at_line_start()
            ), "BUG: reducing text width, when not at a line start"
    ...
    def blank_line(self):
        ...
        assert (
            self.at_line_start()
        ), "BUG: creating a blank line, but not at the start of a line"
    ...
    def advance_page(self):
        ...
        if self.cursor.embossing():
            assert (
                self.cursor.at_page_start()
            ), "Page advance did not reach exactly the start of a new page"

    def write_word(self, word):
        # This assumes there is room on the current line
        # so no adjustments are made to the cursor
        assert (
            self.line_buffer.remaining_chars() == self.cursor.remaining_characters()
        ), "Cursor and LineBuffer have desynced"
    ...
    def process_segment(self, s):
        ...
        assert (
            self.at_line_start()
        ), "BUG: starting a segment, but not at the start of a line"
    ...
    def massage_math(self, aline, punctuation):
        ...
        if math_len <= width:
            ...
            aline = (
                nemeth_open + "\xA0" + aline.replace(" ", "\xA0") + " " + nemeth_close
            )
    ...
    def process_block(self, blk):
        ...
        if blk.box == "standard":
            ...
            open_brf = louis.translateString(
                ["en-ueb-g2.ctb"], BRF.nemeth_open, None, 0
            )
