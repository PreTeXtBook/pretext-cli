...
class Project(pxml.BaseXmlModel, tag="project", search_mode=SearchMode.UNORDERED):
    ...
    def site_strategy(self) -> t.Literal["default_target", "pelican_default", "pelican_custom", "static"]:
        if len(self.deploy_targets()) == 0:
            return "default_target"
        if not self.site_abspath().exists():
            return "pelican_default"
        if (self.site_abspath() / "config.ptx").exists():
    ...
    def stage_deployment(self) -> None:
        ...
        if strategy == "default_target":
            ...
            # Stage the static site directory. (This is done last to overwrite any files generated by targets.)
            if strategy == "static":
                log.info(
