                ...
                if (
                    len(
                        [
                            ...
                            if t.deploy_dir_abspath() == self.stage_abspath()
                            and t.format == "html"
                        ]
                    )
                    == 0
                    ...
                    log.warning(
                        "You have a target with a `deploy-dir` attribute in your project.ptx file,"
                    )
                    log.warning(
                        f"but `{self.site}` does not exist and no HTML-format target is pointed to the root."
                    )
                    log.warning("Your deployment may result in a 404 error.")
            else:
                log.info(
