    ...
    def deploy(
        ...
        for target in self.deploy_targets():
            ...
            if not target.output_dir_abspath().exists():
                ...
                log.info(f"Staging `{target.name}` at `{self.stage_abspath() / deploy_dir}`.")
        ...
        if len(self.deploy_targets()) == 0:
            ...
            if not target.output_dir_abspath().exists():
                ...
                return
            log.info(f"Staging latest build located in `{target.output_dir_abspath()}` at `{self.stage_abspath()}`.")
            log.info("")
            shutil.copytree(
                target.output_dir_abspath(),
                self.stage_abspath(),
        ...
        # Otherwise, stage the site directory. (This is done last to overwrite any files generated by targets.)
            ...
            if not self.site_abspath().exists():
                ...
                log.info(f"Staging custom site located in `{self.site.resolve()}` at `{self.stage_abspath()}`.")
                shutil.copytree(
                    self.site.resolve(),
                    self.stage_abspath(),
                    dirs_exist_ok=True,
