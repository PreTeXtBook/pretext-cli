...
class Project(pxml.BaseXmlModel, tag="project", search_mode=SearchMode.UNORDERED):
    ...
    def update_boilerplate(self, skip_unmanaged: bool = True,) -> None:
        """
        ...
        for resource in constants.PROJECT_RESOURCES:
            ...
            if not project_resource_path.exists() and (
                self.is_git_managed() or resource not in constants.GIT_RESOURCES
            ):
                log.warning(f"Resource {resource} is missing and will be generated.")
                self.generate_boilerplate(resources=[resource])
                continue
            # Since file exists, we read its content
            with open(project_resource_path, "rb") as f:
                contents = f.read()
                lines = contents.decode().splitlines()
            if utils.is_unmodified(resource, contents, resource_hash_table):
                log.info(
