...
class Target:
    ...
    def source(self):
        return os.path.abspath(
            os.path.join(
                self.__project_path, self.xml_element().find("source").text.strip()
            )
        )
    ...
    def publication(self):
        return os.path.abspath(
            os.path.join(
                self.__project_path, self.xml_element().find("publication").text.strip()
            )
        )
    ...
    def webwork_representations_path(self):
        ...
        if dir_ele is None:
            log.debug(
                "Publication file does not specify webwork-representation.ptx file"
            )
            return None
    ...
    def output_dir(self):
        return os.path.abspath(
            os.path.join(
                self.__project_path, self.xml_element().find("output-dir").text.strip()
            )
        )
    ...
    def xsl_path(self):
        if self.xml_element().find("xsl") is not None:
            return os.path.abspath(
                os.path.join(
                    self.__project_path, self.xml_element().find("xsl").text.strip()
                )
            )
        else:
            return None
...
class Project:
    def __init__(self, xml_element=None, targets=None, project_path=None):
        if project_path is None:
            project_path = os.getcwd()
        if xml_element is None:
            if utils.project_path() is not None:
                xml_element = ET.parse(
                    os.path.join(utils.project_path(), "project.ptx")
                ).getroot()
            else:
                template_xml = static.path("templates", "project.ptx")
    ...
    def view(self, target_name, access, port, watch=False, build=False):
        ...
        if not utils.directory_exists(target.output_dir()):
            log.error(
                f"The directory `{target.output_dir()}` does not exist. Maybe try `pretext build {target.name()}` first?"
            )
            return

        def watch_callback(target_name):
            self.build(target_name)
    ...
    def build(
        ...
        if len(target.source_xml().xpath("//webwork")) > 0:
            if target.webwork_representations_path() is None:
                log.warning(
                    'Your source contains WeBWorK exercises but you do not have a "webwork-representations" file specified in your publication file.  Modify your publication file and run `pretext build -w`.'
                )
            elif not os.path.isfile(target.webwork_representations_path()):
                log.warning(
                    'Your source contains WeBWorK exercises the path to the "webwork-representations.ptx" file in your publication file does not point to a file. Run `pretext build -w` or modify your publication file.'
                )
        ...
        if clean:
            if Path(self.__project_path) not in Path(target.output_dir()).parents:
                ...
                in Path(os.path.join(target.source_dir(), "foo")).parents
                or Path(target.output_dir())
                in Path(os.path.join(target.publication_dir(), "foo")).parents
            ):
                log.warning(
        ...
        if diagrams:
            ...
            if (
                target.format() == "html"
                and len(source_xml.xpath("//asymptote|//latex-image|//sageplot")) > 0
            ):
                log.warning(
                    "The source has generated images (<latex-image/>, <asymptote/>, or <sageplot/>), "
            ...
            if (
                ...
                and len(
                    source_xml.xpath(
                        "//asymptote|//sageplot|//video[@youtube]|//interactive[not(@preview)]"
                    )
                )
                > 0
            ):
                log.warning(
                    "The source has interactive elements or videos that need a preview to be generated, "
                    + "but these will not be (re)built. Run `pretext build` with the `-d` flag if updates are needed."
                )
        try:
            if (
                target.format() == "html" or target.format() == "html-zip"
            ) and not only_assets:
                ...
                shutil.copytree(
                    target.external_dir(), os.path.join(target.output_dir(), "external")
                )
                shutil.copytree(
                    target.generated_dir(),
                    os.path.join(target.output_dir(), "generated"),
                )
    ...
    def publish(self, target_name, commit_message="Update to PreTeXt project source."):
        ...
        if not utils.directory_exists(target.output_dir()):
            log.error(
                f"The directory `{target.output_dir()}` does not exist. Maybe try `pretext build` first?"
            )
            return
        log.info(
