...
def latex(ptxfile, pub_file, output, stringparams, custom_xsl):
    ...
    with utils.working_directory("."):
        core.latex(
            ptxfile, utils.linux_path(pub_file), stringparams, custom_xsl, None, output
        )
...
def diagrams(
    ...
    if (
        ...
        and formats[target_format]["latex-image"] is not None
    ):
        ...
        with utils.working_directory("."):
            for outformat in formats[target_format]["latex-image"]:
                core.latex_image_conversion(
                    xml_source=ptxfile,
                    pub_file=utils.linux_path(pub_file),
                    stringparams=params,
                    xmlid_root=xmlid_root,
                    dest_dir=image_output,
                    outformat=outformat,
                    method=pdf_method,
                )
    if (
        len(source_xml.xpath("/pretext/*[not(docinfo)]//sageplot")) > 0
        and formats[target_format]["sageplot"] is not None
    ):
    ...
    if (
        ...
        and formats[target_format]["asymptote"]
    ):
        image_output = os.path.abspath(os.path.join(output, "asymptote"))
        utils.ensure_directory(image_output)
    ...
    if (
        len(source_xml.xpath("/pretext/*[not(docinfo)]//interactive[not(@preview)]"))
        > 0
    ):
        image_output = os.path.abspath(os.path.join(output, "preview"))
        utils.ensure_directory(image_output)
    ...
    if len(source_xml.xpath("/pretext/*[not(docinfo)]//video[@youtube]")) > 0:
        image_output = os.path.abspath(os.path.join(output, "youtube"))
        utils.ensure_directory(image_output)
